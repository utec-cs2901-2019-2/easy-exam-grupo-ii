...
public interface IProblemSubmittedRepo extends JpaRepository<ProblemSubmitted, ProblemSubmittedId> {
    ...
    @Query (value = "select users.id, users.email, problem.id from users,problem, problem_submitted ps where users.id = ps.id_user and ps.id_problem = problem.id and users.id = ?1", nativeQuery = true)
